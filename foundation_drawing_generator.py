...
DIM_OFFSET_1 = 2200         # individual span dimensions (pushed down for footings)
DIM_OFFSET_2 = 3200         # overall span dimensions
GRID_LABEL_OFFSET = 4200    # center of grid label circles
GRID_LINE_END = 4650        # grid lines terminate here
...
def read_gridline_coordinates(filepath, floor_filter="Foundation"):
    """Read gridline_coordinates Excel. Returns list of column dicts for specified floor."""
    wb = openpyxl.load_workbook(filepath, read_only=True)
    ws = wb.active
    columns = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        floor_type = row[2]
        if floor_filter and floor_type != floor_filter:
            continue
        columns.append({
            "no": row[0],
            "name": row[1],
            "location": row[4],
            "anchor_x": row[5] * SCALE,
            "anchor_y": row[6] * SCALE,
            "beam_x_loc": row[7],
            "beam_y_loc": row[8],
            "orientation": row[9],
            "yd": row[10],
...
FOUNDATION_INPUT = os.path.join(STD_ANL_FOLDER, "Foundation_integrated_outcome.xlsx")


def read_isolated_footings(filepath):
    """Read Isolated Footings sheet. Returns list of footing dicts."""
    wb = openpyxl.load_workbook(filepath, read_only=True)
    if "Isolated Footings" not in wb.sheetnames:
        wb.close()
        return []
    ws = wb["Isolated Footings"]
    footings = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        footings.append({
            "id": row[0],
            "member": row[1],
            "x_min": float(row[2]) * SCALE,
            "x_max": float(row[3]) * SCALE,
            "z_min": float(row[4]) * SCALE,
            "z_max": float(row[5]) * SCALE,
            "L": row[6],
            "B": row[7],
            "h": row[8],
        })
...
def read_combined_footings(filepath):
    """Read Combined Footings sheet. Returns list of footing dicts."""
    wb = openpyxl.load_workbook(filepath, read_only=True)
    if "Combined Footings" not in wb.sheetnames:
        wb.close()
        return []
    ws = wb["Combined Footings"]
    footings = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        if row[0] is None or row[0] == "TOTAL":
            continue
...
def draw_footings(msp, footings):
    """Draw footing rectangles, labels, and L/B dimension lines."""
    dim_off = 200  # offset of dimension line from footing edge
    ar = 60        # arrow size
    th = 100       # text height
    lyr = {"layer": "FOOTING_DIM"}

    for f in footings:
        x1, x2 = f["x_min"], f["x_max"]
        y1, y2 = f["z_min"], f["z_max"]
        # Footing rectangle
        msp.add_lwpolyline(
            [(x1, y1), (x2, y1), (x2, y2), (x1, y2)],
            dxfattribs={"layer": "FOOTING"}, close=True)
        # Label at center
        cx = (x1 + x2) / 2
        cy = (y1 + y2) / 2
        msp.add_text(f["id"], height=150,
                     dxfattribs={"layer": "FOOTING_LABEL"}).set_placement(
            (cx, cy), align=TextEntityAlignment.MIDDLE_CENTER)

        # L dimension (horizontal, below footing)
        if "L" in f and f["L"]:
            l_mm = float(f["L"]) * SCALE
            dy = y1 - dim_off
            msp.add_line((x1, dy), (x2, dy), dxfattribs=lyr)
            # Left arrow
            msp.add_line((x1, dy), (x1 + ar, dy + ar * 0.4), dxfattribs=lyr)
            msp.add_line((x1, dy), (x1 + ar, dy - ar * 0.4), dxfattribs=lyr)
            # Right arrow
            msp.add_line((x2, dy), (x2 - ar, dy + ar * 0.4), dxfattribs=lyr)
            msp.add_line((x2, dy), (x2 - ar, dy - ar * 0.4), dxfattribs=lyr)
            # Text
            msp.add_text(_dim_text(l_mm), height=th, dxfattribs=lyr).set_placement(
                (cx, dy + th * 0.3), align=TextEntityAlignment.BOTTOM_CENTER)

        # B dimension (vertical, right of footing)
        if "B" in f and f["B"]:
            b_mm = float(f["B"]) * SCALE
            dx = x2 + dim_off
            msp.add_line((dx, y1), (dx, y2), dxfattribs=lyr)
            # Bottom arrow
            msp.add_line((dx, y1), (dx + ar * 0.4, y1 + ar), dxfattribs=lyr)
            msp.add_line((dx, y1), (dx - ar * 0.4, y1 + ar), dxfattribs=lyr)
            # Top arrow
            msp.add_line((dx, y2), (dx + ar * 0.4, y2 - ar), dxfattribs=lyr)
            msp.add_line((dx, y2), (dx - ar * 0.4, y2 - ar), dxfattribs=lyr)
            # Text
            msp.add_text(_dim_text(b_mm), height=th, rotation=90, dxfattribs=lyr).set_placement(
                (dx - th * 0.3, cy), align=TextEntityAlignment.BOTTOM_CENTER)


def read_strap_beams(filepath):
    """Read Strap Beams sheet. Returns list of strap beam dicts."""
    wb = openpyxl.load_workbook(filepath, read_only=True)
    if "Strap Beams" not in wb.sheetnames:
        wb.close()
        return []
    ws = wb["Strap Beams"]
    beams = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        if row[0] is None or row[0] == "TOTAL":
            continue
        members = str(row[1]).strip()
        parts = members.split("-")
        if len(parts) != 2:
            continue
        beams.append({
            "id": row[0],
            "member_left": int(parts[0].strip()),
            "member_right": int(parts[1].strip()),
            "clear_span": float(row[2]),
            "left_in": float(row[3]) * SCALE,
            "right_in": float(row[4]) * SCALE,
            "b": float(row[5]),  # mm
            "h": float(row[6]),  # mm
        })
    wb.close()
...
def draw_strap_beams(msp, strap_beams, columns, footings_list=None):
    """Draw strap beams between columns. Beam runs horizontally between two columns."""
    # Build column lookup by number
    col_map = {}
    for c in columns:
        col_map[c["no"]] = c

    # Build footing lookup by member number
    footing_map = {}
    if footings_list:
        for f in footings_list:
            member = f.get("member")
            members_str = f.get("members", "")
            if member:
                try:
                    footing_map[int(member)] = f
                except (ValueError, TypeError):
                    pass
            if members_str:
                # Combined footing — map each member to the same footing
                for m in str(members_str).replace("+", " ").split():
                    try:
                        footing_map[int(m.strip())] = f
                    except (ValueError, TypeError):
                        pass

    for sb in strap_beams:
        left_col = col_map.get(sb["member_left"])
        right_col = col_map.get(sb["member_right"])
        if not left_col or not right_col:
            print(f"    WARNING: Strap beam {sb['id']} — column {sb['member_left']} or {sb['member_right']} not found")
            continue

        # Determine beam direction
        # Check if columns share similar Y (horizontal beam) or similar X (vertical beam)
        left_cx = (left_col["x1"] + left_col["x2"]) / 2
        left_cy = (left_col["y1"] + left_col["y2"]) / 2
        right_cx = (right_col["x1"] + right_col["x2"]) / 2
        right_cy = (right_col["y1"] + right_col["y2"]) / 2

        dx = abs(right_cx - left_cx)
        dy = abs(right_cy - left_cy)
        half_b = sb["b"] / 2

        if dx >= dy:
            # Horizontal beam
            # X: from left column right face to right column left face (clear span only)
            beam_x1 = left_col["x2"]
            beam_x2 = right_col["x1"]
            # Y: centered on average Y of both columns, width = b
            beam_cy = (left_cy + right_cy) / 2
            beam_y1 = beam_cy - half_b
            beam_y2 = beam_cy + half_b
        else:
            # Vertical beam
            beam_y1 = left_col["y2"]
            beam_y2 = right_col["y1"]
            beam_cx = (left_cx + right_cx) / 2
            beam_x1 = beam_cx - half_b
            beam_x2 = beam_cx + half_b

        # Draw beam rectangle
        msp.add_lwpolyline(
            [(beam_x1, beam_y1), (beam_x2, beam_y1), (beam_x2, beam_y2), (beam_x1, beam_y2)],
            dxfattribs={"layer": "STRAP_BEAM"}, close=True)

        # Label at center
        cx = (beam_x1 + beam_x2) / 2
        cy = (beam_y1 + beam_y2) / 2
        msp.add_text(sb["id"], height=150,
                     dxfattribs={"layer": "STRAP_BEAM"}).set_placement(
            (cx, cy), align=TextEntityAlignment.MIDDLE_CENTER)

        # Dimension markings — horizontal arrows showing left_in and right_in at footing zones
        ar_sb = 60
        th_sb = 100
        lyr_sb = {"layer": "FOOTING_DIM"}

        if dx >= dy:
            # Horizontal beam — dimension = strap beam portion inside each footing
            dim_y = beam_y2 + 150  # above the strap beam

            # Left side: strap beam portion inside left footing
            left_ftg = footing_map.get(sb["member_left"])
            if left_ftg:
                lx1 = max(beam_x1, left_ftg["x_min"])  # where strap enters footing
                lx2 = min(beam_x2, left_ftg["x_max"])  # where strap exits footing
                dim_len = abs(lx2 - lx1)
                msp.add_line((lx1, dim_y), (lx2, dim_y), dxfattribs=lyr_sb)
                msp.add_line((lx1, dim_y), (lx1 + ar_sb, dim_y + ar_sb * 0.4), dxfattribs=lyr_sb)
                msp.add_line((lx1, dim_y), (lx1 + ar_sb, dim_y - ar_sb * 0.4), dxfattribs=lyr_sb)
                msp.add_line((lx2, dim_y), (lx2 - ar_sb, dim_y + ar_sb * 0.4), dxfattribs=lyr_sb)
                msp.add_line((lx2, dim_y), (lx2 - ar_sb, dim_y - ar_sb * 0.4), dxfattribs=lyr_sb)
                msp.add_text(_dim_text(dim_len), height=th_sb, dxfattribs=lyr_sb).set_placement(
                    ((lx1 + lx2) / 2, dim_y + th_sb * 0.5), align=TextEntityAlignment.BOTTOM_CENTER)

            # Right side: strap beam portion inside right footing
            right_ftg = footing_map.get(sb["member_right"])
            if right_ftg:
                rx1 = max(beam_x1, right_ftg["x_min"])  # where strap enters footing
                rx2 = min(beam_x2, right_ftg["x_max"])  # where strap exits footing
                dim_len = abs(rx2 - rx1)
                msp.add_line((rx1, dim_y), (rx2, dim_y), dxfattribs=lyr_sb)
                msp.add_line((rx1, dim_y), (rx1 + ar_sb, dim_y + ar_sb * 0.4), dxfattribs=lyr_sb)
                msp.add_line((rx1, dim_y), (rx1 + ar_sb, dim_y - ar_sb * 0.4), dxfattribs=lyr_sb)
                msp.add_line((rx2, dim_y), (rx2 - ar_sb, dim_y + ar_sb * 0.4), dxfattribs=lyr_sb)
                msp.add_line((rx2, dim_y), (rx2 - ar_sb, dim_y - ar_sb * 0.4), dxfattribs=lyr_sb)
                msp.add_text(_dim_text(dim_len), height=th_sb, dxfattribs=lyr_sb).set_placement(
                    ((rx1 + rx2) / 2, dim_y + th_sb * 0.5), align=TextEntityAlignment.BOTTOM_CENTER)


def draw_outer_wall(msp, columns, wall_thickness=230):
    """Draw 230mm thick wall along outer boundary. Outer line matches column outer face,
    inner line is 230mm inward."""
    x_min = min(c["x1"] for c in columns)
    x_max = max(c["x2"] for c in columns)
    y_min = min(c["y1"] for c in columns)
    y_max = max(c["y2"] for c in columns)

    wt = wall_thickness
    lyr = {"layer": "WALL"}

    hatch_lyr = {"layer": "WALL_HATCH", "color": 9}

    def _add_wall_segment(x1, y1, x2, y2):
        """Draw wall outline and diagonal hatch fill."""
        # Outline
        msp.add_line((x1, y1), (x2, y1), dxfattribs=lyr)
        msp.add_line((x2, y1), (x2, y2), dxfattribs=lyr)
        msp.add_line((x2, y2), (x1, y2), dxfattribs=lyr)
        msp.add_line((x1, y2), (x1, y1), dxfattribs=lyr)
        # Hatch
        hatch = msp.add_hatch(dxfattribs={"layer": "WALL_HATCH", "color": 9})
        hatch.set_pattern_fill("ANSI31", scale=15.0)
        hatch.paths.add_polyline_path(
            [(x1, y1), (x2, y1), (x2, y2), (x1, y2)], is_closed=True)

    # Left wall
    _add_wall_segment(x_min, y_min, x_min + wt, y_max)

    # Right wall
    _add_wall_segment(x_max - wt, y_min, x_max, y_max)

    # Bottom wall (between left and right walls)
    _add_wall_segment(x_min + wt, y_min, x_max - wt, y_min + wt)

    # Top wall (between left and right walls)
    _add_wall_segment(x_min + wt, y_max - wt, x_max - wt, y_max)


def draw_section_marker(msp, cx, cy, number, direction, radius=150):
    """Draw a section marker: circle with number + small triangle pointing in view direction."""
    import math
    lyr = {"layer": "SECTION_MARK"}
    lyr_text = {"layer": "SECTION_TEXT"}
    r = radius
    tri_h = r * 0.7
    tri_w = r * 0.5

    # Circle
    msp.add_circle((cx, cy), r, dxfattribs=lyr)

    # Number inside circle
    msp.add_text(str(number), height=r * 1.0,
                 dxfattribs=lyr_text).set_placement(
        (cx, cy), align=TextEntityAlignment.MIDDLE_CENTER)

    # Triangle on the viewing direction side
    if direction == "up":
        tip = (cx, cy + r + tri_h)
        base_l = (cx - tri_w, cy + r)
        base_r = (cx + tri_w, cy + r)
    elif direction == "down":
        tip = (cx, cy - r - tri_h)
        base_l = (cx - tri_w, cy - r)
        base_r = (cx + tri_w, cy - r)
        ...
        tip = (cx - r - tri_h, cy)
        base_l = (cx - r, cy - tri_w)
        base_r = (cx - r, cy + tri_w)
    ...
    msp.add_solid([tip, base_l, base_r], dxfattribs={"layer": "SECTION_MARK", "color": 9})
    msp.add_lwpolyline([tip, base_l, base_r], dxfattribs=lyr, close=True)


def draw_section_line(msp, mx, my, fx, fy):
    """Draw an L-shaped line from marker to footing edge.
    Goes vertical first, then horizontal (or vice versa)."""
    lyr = {"layer": "SECTION_LINE"}
    # Vertical then horizontal
    msp.add_line((mx, my), (mx, fy), dxfattribs=lyr)
    msp.add_line((mx, fy), (fx, fy), dxfattribs=lyr)


def draw_all_sections(msp, footings, combined, columns, grid_x, grid_y):
    """Draw all section markers (1-1 through 6-6)."""
    # Build column lookup by member
    col_map = {}
    for c in columns:
        col_map[c["no"]] = c

    # Building boundary
    bx_min = min(c["x1"] for c in columns)
    bx_max = max(c["x2"] for c in columns)
    by_min = min(c["y1"] for c in columns)
    by_max = max(c["y2"] for c in columns)
    mid_x = (bx_min + bx_max) / 2

    mo = 500  # marker offset from footing edge

    # ── Section 4-4: Edge footings — horizontal cut along column grid line ──
    for f in footings:
        if f.get("id") == "IF6":
            continue
        member = f.get("member")
        if not member:
            continue
        col = col_map.get(int(member))
        if not col:
            continue

        col_cy = (col["y1"] + col["y2"]) / 2
        fx1, fx2 = f["x_min"], f["x_max"]
        fy1, fy2 = f["z_min"], f["z_max"]
        fcx = (fx1 + fx2) / 2
        ...
        draw_section_marker(msp, fx1 - mo, my, 4, d)
        draw_section_line(msp, fx1 - mo, my, fx1, col_cy)
        draw_section_marker(msp, fx2 + mo, my, 4, d)
        draw_section_line(msp, fx2 + mo, my, fx2, col_cy)

    # ── Section 1-1: Interior footing (IF6) ──
    for f in footings:
        if f.get("id") != "IF6":
            continue
        member = f.get("member")
        col = col_map.get(int(member)) if member else None
        if not col:
            continue
        col_cx = (col["x1"] + col["x2"]) / 2
        fy1, fy2 = f["z_min"], f["z_max"]

        draw_section_marker(msp, col_cx, fy1 - mo, 1, "left")
        draw_section_line(msp, col_cx, fy1 - mo, col_cx, fy1)
        draw_section_marker(msp, col_cx, fy2 + mo, 1, "right")
        draw_section_line(msp, col_cx, fy2 + mo, col_cx, fy2)

    # ── Section 3-3: Combined footing — vertical cut ──
    for f in combined:
        fx1, fx2 = f["x_min"], f["x_max"]
        fy1, fy2 = f["z_min"], f["z_max"]
        fcx = (fx1 + fx2) / 2

        draw_section_marker(msp, fcx, fy1 - mo, 3, "left")
        draw_section_line(msp, fcx, fy1 - mo, fcx, fy1)
        draw_section_marker(msp, fcx, fy2 + mo, 3, "right")
        draw_section_line(msp, fcx, fy2 + mo, fcx, fy2)

    # ── Section 2-2: Combined footing — horizontal cut ──
    for f in combined:
        fx1, fx2 = f["x_min"], f["x_max"]
        fy1, fy2 = f["z_min"], f["z_max"]
        fcy = (fy1 + fy2) / 2

        draw_section_marker(msp, fx1 - mo, fcy, 2, "up")
        draw_section_line(msp, fx1 - mo, fcy, fx1, fcy)
        draw_section_marker(msp, fx2 + mo, fcy, 2, "down")
        draw_section_line(msp, fx2 + mo, fcy, fx2, fcy)

    # ── Section 5-5: Wall — bottom and top ──
    wmx = (bx_min + bx_max) / 2
    draw_section_marker(msp, wmx, by_min - mo, 5, "left")
    draw_section_line(msp, wmx, by_min - mo, wmx, by_min)
    draw_section_marker(msp, wmx, by_max + mo, 5, "right")
    draw_section_line(msp, wmx, by_max + mo, wmx, by_max)

    # ── Section 6-6: Wall — left and right ──
    wmy = (by_min + by_max) / 2
    draw_section_marker(msp, bx_min - mo, wmy, 6, "up")
    draw_section_line(msp, bx_min - mo, wmy, bx_min, wmy)
    draw_section_marker(msp, bx_max + mo, wmy, 6, "down")
    draw_section_line(msp, bx_max + mo, wmy, bx_max, wmy)
...
def setup_document():
    ...
    # Add DASHED linetype for section cut lines
    ...
    doc.layers.add("FOOTING", color=50)
    doc.layers.add("FOOTING_LABEL", color=CLR_GREEN)
    doc.layers.add("STRAP_BEAM", color=221)
    doc.layers.add("WALL", color=CLR_RED)
    doc.layers.add("WALL_HATCH", color=9)
    doc.layers.add("SECTION_MARK", color=CLR_YELLOW)
    doc.layers.add("SECTION_TEXT", color=CLR_GREEN)
    doc.layers.add("SECTION_LINE", color=CLR_YELLOW)
...
def read_fck_from_global_assumptions():
    """Read Fck value from Global_assumptions.csv."""
    import csv
    csv_path = os.path.join(r"D:\JARVIS back up 16092025\JARVIS backup\Code - Full set", "Global_assumptions.csv")
...
def draw_notes_panel(msp, notes_x, notes_y, notes_h, fck=25):
    ...
    notes = [
        ...
        "G4. MAIN BAR IS ON TOP FACE WHILE THE CROSS BAR",
        "    IS ON BOTTOM FACE.",
    ]
    for line in notes:
    ...
    msp.add_text("CONCRETE:-", height=hh, dxfattribs={"layer": layer_hdr}).set_placement(
        (x + 150, cy), align=TextEntityAlignment.TOP_LEFT)
    cy -= hh + line_sp
    msp.add_text(f"ALL R.C.C WORK SHALL BE IN MIX M{fck}", height=th,
...
def draw_main_title(msp, grid_x, grid_y):
    """Draw 'FOUNDATION PLAN' title centered below the grid label circles."""
    ...
    msp.add_text("FOUNDATION PLAN", height=500,
                 dxfattribs={"layer": "TITLE"}).set_placement(
...
def main():
    print("=" * 60)
    print("  FOUNDATION DRAWING GENERATOR")
    print("=" * 60)
    print()
    ...
    print("  3x. Outer wall boundary")
    draw_outer_wall(msp, columns)

    print("  3. Column rectangles")
    draw_column_rectangles(msp, columns)

    # Isolated footings
    footings = read_isolated_footings(FOUNDATION_INPUT)
    if footings:
        print(f"  3a. Isolated footings ({len(footings)})")
        draw_footings(msp, footings)

    # Combined footings
    combined = read_combined_footings(FOUNDATION_INPUT)
    if combined:
        print(f"  3b. Combined footings ({len(combined)})")
        draw_footings(msp, combined)

    # Strap beams
    strap_beams = read_strap_beams(FOUNDATION_INPUT)
    if strap_beams:
        print(f"  3c. Strap beams ({len(strap_beams)})")
        all_footings = footings + combined
        draw_strap_beams(msp, strap_beams, columns, all_footings)
    ...
    # Section markers
    print("  4a. Section markers")
    draw_all_sections(msp, footings if footings else [], combined if combined else [], columns, grid_x, grid_y)

    print("  6. Grid dimensions")
    draw_dimensions(msp, grid_x, grid_y, registry=registry)

    # Column-related dimensions — disabled for foundation drawing
    ...
    # Building perimeter — disabled for foundation drawing
    # print("  7. Building perimeter")
    # draw_perimeter(msp, columns)

    # Plot boundary — disabled for foundation drawing
    ...
    if notes_h < 5000:
        notes_h = 5000
    fck = read_fck_from_global_assumptions()
    draw_notes_panel(msp, panel_x, notes_y, notes_h, fck=fck)
    ...
    outpath = os.path.join(OUTPUT_FOLDER, f"foundation_plan_{timestamp}.dxf")
