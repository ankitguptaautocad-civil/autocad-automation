...
from dxf_styles import setup_dimstyles
...
def _to_float(value, default=0.0):
    """Convert Excel cell values to float, tolerating numeric strings."""
    if value is None:
        return default
...
def read_slab_panels(filepath):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        panel = {
            "rect_no": row[1],
            "lx": _to_float(row[2]),
            "clear_lx": _to_float(row[3]),
            "lz": _to_float(row[4]),
            "clear_lz": _to_float(row[5]),
            "thickness": _to_float(row[6]),
            "x_start": _to_float(row[7]),
            "x_end": _to_float(row[8]),
            "z_start": _to_float(row[9]),
            "z_end": _to_float(row[10]),
            "zone": zone,
        }
...
def read_slab_panels_by_y(filepath):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        if row[0] is None:
            continue
        y_val = _to_float(row[0])
        zone = row[11]
        if zone and str(zone).strip() in ("None", ""):
            zone = None
        panel = {
            "rect_no": row[1],
            "lx": _to_float(row[2]), "clear_lx": _to_float(row[3]),
            "lz": _to_float(row[4]), "clear_lz": _to_float(row[5]),
            "thickness": _to_float(row[6]),
            "x_start": _to_float(row[7]), "x_end": _to_float(row[8]),
            "z_start": _to_float(row[9]), "z_end": _to_float(row[10]),
            "zone": zone,
            "floor_type": row[12],
        }
...
def _norm_floor_type_key(value):
    """Normalize floor-type labels for tolerant matching."""
    if value is None:
        return ""
...
def read_gridline_coordinates(filepath):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        floor_type = row[2]
        floor_type_key = _norm_floor_type_key(floor_type)
        raw_x1 = row[12] * SCALE
        raw_x2 = row[13] * SCALE
        raw_y1 = row[14] * SCALE
        raw_y2 = row[15] * SCALE
        col = {
            "no": row[0], "name": row[1],
            "floor_type": floor_type, "floor_type_key": floor_type_key, "y_value": row[3],
            "location": row[4],
        ...
        all_columns.setdefault(floor_type_key, []).append(col)
    wb.close()
    return all_columns


def get_columns_for_floor_type(all_columns, floor_type, fallbacks=None):
    """Fetch columns for a floor type with normalized matching and optional fallbacks."""
    for candidate in [floor_type] + list(fallbacks or []):
        cols = all_columns.get(_norm_floor_type_key(candidate), [])
        if cols:
            return cols
    return []
...
def get_columns_for_floor(all_columns, beam_floor):
    ...
    # floor_map = {
    #     "Stilt": "Stilt floor",
    #     "Typical": "First floor roof",
    #     "Terrace": "Terrace",
    #     "Mumty": "Mumty roof",
    # }
    floor_map = {
        "Stilt": "Plinth",
        "Typical": "1st floor roof",
        "Terrace": "Stilt roof",      # Terrace no longer separate; use Stilt roof
        "Mumty": "Mumty",
    }
    floor_type = floor_map.get(beam_floor, beam_floor)
    return get_columns_for_floor_type(all_columns, floor_type, fallbacks=["Plinth"])
...
def draw_beams(msp, floor_beams, columns, ox, oy):
    ...
    if not columns:
        print("      Skipping balcony closing lines: no columns available for this floor")
        return
...
def _add_linear_dim(msp, *, base, p1, p2, angle, text):
    """Wrapper around msp.add_linear_dim that always renders + uses S-DIM-50."""
...
def draw_dimensions(msp, grid_x, grid_y, ox, oy):
    """Draw grid spacing dimensions at top (X) and left (Y) as native DIMENSIONs."""
    min_x, max_x = min(grid_x), max(grid_x)
    min_y, max_y = min(grid_y), max(grid_y)

    # X dims at top — individual segments
    y_dim = max_y + DIM_OFFSET_X1
    for i in range(len(grid_x) - 1):
        x1, x2 = grid_x[i], grid_x[i + 1]
        _add_linear_dim(
            msp,
            base=(ox + (x1 + x2) / 2, oy + y_dim),
            p1=(ox + x1, oy + max_y),
            p2=(ox + x2, oy + max_y),
            angle=0,
            text=_dim_text(x2 - x1),
        )
    ...
    if len(grid_x) > 2:
        ...
        _add_linear_dim(
            msp,
            base=(ox + (grid_x[0] + grid_x[-1]) / 2, oy + y_dim),
            p1=(ox + grid_x[0], oy + max_y),
            p2=(ox + grid_x[-1], oy + max_y),
            angle=0,
            text=_dim_text(grid_x[-1] - grid_x[0]),
        )

    # Y dims at left — individual segments
    x_dim = min_x - DIM_OFFSET_Y1
    for i in range(len(grid_y) - 1):
        y1, y2 = grid_y[i], grid_y[i + 1]
        _add_linear_dim(
            msp,
            base=(ox + x_dim, oy + (y1 + y2) / 2),
            p1=(ox + min_x, oy + y1),
            p2=(ox + min_x, oy + y2),
            angle=90,
            text=_dim_text(y2 - y1),
        )
    ...
    if len(grid_y) > 2:
        ...
        _add_linear_dim(
            msp,
            base=(ox + x_dim, oy + (grid_y[0] + grid_y[-1]) / 2),
            p1=(ox + min_x, oy + grid_y[0]),
            p2=(ox + min_x, oy + grid_y[-1]),
            angle=90,
            text=_dim_text(grid_y[-1] - grid_y[0]),
        )
...
def draw_rebar_for_panel(msp, panel, rebar_spec, ox, oy):
    ...
    is_balcony = panel.get("zone") in ("BB1", "BF1")

    # ── Balcony: straight rebars only, no cranking ──
    if is_balcony:
        # Horizontal bars
        bar_y1 = y1 + panel_h / 2 + gap
        bar_y2 = y1 + panel_h / 2 - gap
        msp.add_line((ox+x1-beam_w, oy+bar_y1), (ox+x2+beam_w, oy+bar_y1),
                     dxfattribs={"layer":"REBAR_BOT"})
        msp.add_line((ox+x1-beam_w, oy+bar_y2), (ox+x2+beam_w, oy+bar_y2),
                     dxfattribs={"layer":"REBAR_BOT"})
        # Vertical bars
        bar_x1 = x1 + panel_w / 2 + gap
        bar_x2 = x1 + panel_w / 2 - gap
        msp.add_line((ox+bar_x1, oy+y1-beam_h), (ox+bar_x1, oy+y2+beam_h),
                     dxfattribs={"layer":"REBAR_BOT"})
        msp.add_line((ox+bar_x2, oy+y1-beam_h), (ox+bar_x2, oy+y2+beam_h),
                     dxfattribs={"layer":"REBAR_BOT"})
        return
...
def main():
    ...
    def y_to_col_type(yv):
        # # ── Old floor type logic (kept for reference) ──
        # y_above_0 = [v for v in all_y_sorted if v > 0]
        # if yv not in y_above_0:
        #     return "Stilt floor"
        # idx = y_above_0.index(yv)
        # if idx == 0:
        #     return "Stilt floor roof"
        # if yv == y_above_0[-1]:
        #     return "Mumty roof"
        # if yv == y_above_0[-2]:
        #     return "Terrace"
        # ord_words = ["First", "Second", ...]
        # return f"{ord_words[floor_num - 1]} floor roof"
        ...
        # Numbered floor roofs: 1st, 2nd, 3rd, etc.
        floor_num = idx  # idx 1 = 1st floor roof
        suffix = {1: "st", 2: "nd", 3: "rd"}.get(floor_num % 10, "th")
        if 11 <= (floor_num % 100) <= 13:
            suffix = "th"
        return f"{floor_num}{suffix} floor roof"
    ...
    for y_val in all_y_sorted:
        ...
        floor_cols = get_columns_for_floor_type(
            all_columns,
            col_type,
            fallbacks=["Stilt roof", "Plinth"],
        )

        # Get beams for this floor
        beam_floor_map = {
            "Stilt": "Stilt", "Mumty_Roof": "Mumty"
        }
        ...
        # Dynamic floor title
        # # ── Old title logic (kept for reference) ──
        # if y_idx == 0: ftitle = "STILT FLOOR ROOF SLAB"
        # elif y_val == y_above_0[-1]: ftitle = "MUMTY ROOF SLAB"
        # elif y_val == y_above_0[-2]: ftitle = "TERRACE FLOOR ROOF SLAB"
        # else: ftitle = f"{n}{suffix} FLOOR ROOF SLAB"
        y_above_0 = [yv for yv in all_y_sorted if yv > 0]
        y_idx = y_above_0.index(y_val) if y_val in y_above_0 else -1
        if y_idx == 0:
            ftitle = "STILT ROOF SLAB"
        ...
        _, dwg_path = save_dxf_and_dwg(doc_f, fpath)
        print(f"  Saved: {os.path.basename(fpath)}"
              + (f" + {os.path.basename(dwg_path)}" if dwg_path else ""))
        generated += 1
