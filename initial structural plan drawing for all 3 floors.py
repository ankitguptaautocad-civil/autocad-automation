...
def normalize_misaligned_primary_beams(primary_beams: list, tolerance_m: float = 0.10) -> list:
    ...
    columns whose grid lines don't actually align, the pipeline records it
    as a single straight member. In real construction this is a beam at
    ONE grid line; the other column is structurally offset and supported
    by its own perpendicular beam. Snap the START coordinate of any such
    beam onto the END column's grid line so the beam draws axially."""
...
def main() -> None:
    ...
    # NOTE: diagonal-beam axial snap now lives upstream in the Appended
    # generator's rewrite_primary_beams, so the corrected coordinates are
    # already persisted in the Primary Beams sheet. Plan drawing is a pure
    # reader of that sheet.
