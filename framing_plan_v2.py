...
def main():
    ...
    # ── Floor 1: STILT ──
    print("\n[STILT LEVEL FRAMING PLAN]")
    stilt_beams = get_floor_beams(all_beams, "Stilt")
    stilt_cols = get_columns_for_floor(all_columns, "Stilt")
    stilt_ox, stilt_oy = 0, 0
    draw_floor_plan(msp, "STILT LEVEL FRAMING PLAN",
                    stilt_beams, grid_x, grid_y, stilt_cols,
    ...
    typ_beams = get_floor_beams(all_beams, "Typical")
    typ_cols = get_columns_for_floor(all_columns, "Typical")
    typ_ox = stilt_ox + draw_w + FLOOR_GAP
    typ_oy = 0
    ...
    # ── Floor 3: TERRACE ──
    print("\n[TERRACE LEVEL FRAMING PLAN]")
    terrace_beams = get_floor_beams(all_beams, "Terrace")
    terrace_cols = get_columns_for_floor(all_columns, "Terrace")
    terrace_ox = typ_ox
    terrace_oy = -(draw_h + FLOOR_GAP)
    terrace_rects = filter_rects_to_mumty_zone(rectangles)
    print(f"  Terrace markings (mumty zone only): {len(terrace_rects)} rectangles")
    draw_floor_plan(msp, "TERRACE LEVEL FRAMING PLAN",
                    terrace_beams, grid_x, grid_y, terrace_cols,
                    terrace_rects, "Stilt floor",
                    terrace_ox, terrace_oy,
    ...
    if not mumty_cols:
        mumty_cols = filter_mumty_columns(
            get_columns_for_floor(all_columns, "Terrace"), rectangles)
    ...
    # Bottom border: lowest of mumty, terrace, or stilt
    lowest_y = stilt_oy + min(grid_y)
    if terrace_oy + min(grid_y) < lowest_y:
        lowest_y = terrace_oy + min(grid_y)
